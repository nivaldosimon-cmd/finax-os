...
Versão adaptada para Streamlit Cloud
"""
...
# ============================================
# CONFIGURAÇÃO DAS CREDENCIAIS (STREAMLIT CLOUD + LOCAL)
# ============================================
def get_supabase_creds():
    """Obtém as credenciais do Supabase (local ou cloud)"""
    try:
...
# Criar arquivo temporário de configuração para os módulos
...
# Importar módulos após configurar credenciais
...
def signup_page():
    ...
    with tab2:
        ...
        escola_id = st.text_input("ID da Escola", placeholder="EX: ESC_ABC123")
        ...
        with col1:
            classe = st.selectbox("Classe", ["10ª", "11ª", "12ª", "9ª", "8ª", "7ª"])
        with col2:
            turma = st.selectbox("Turma", ["A", "B", "C", "D"])
        with col3:
            curso = st.selectbox("Curso", ["Ciências", "Humanidades", "Técnico"])
        
        if st.button("📝 Cadastrar Estudante", use_container_width=True):
            if all([escola_id, nome, username, password]):
                try:
                ...
                st.warning("Preencha ID da escola, nome, username e password!")
...
def dashboard_home(user, supabase, escola_id):
    ...
    with col2:
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-value">{total_presencas}</div>
            <div class="metric-label">📍 Presenças Hoje</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col3:
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-value">{alunos_debito}</div>
            <div class="metric-label">💰 Alunos com Débito</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col4:
        ...
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-value">{percentual:.0f}%</div>
    ...
    with col1:
        ...
        if alunos.data:
            ...
            for a in alunos.data:
                turma_key = f"{a.get('classe', 'N/A')} {a.get('turma', '')}"
                turmas[turma_key] = turmas.get(turma_key, 0) + 1
            
            df = pd.DataFrame(list(turmas.items()), columns=['Turma', 'Quantidade'])
            fig = px.bar(df, x='Turma', y='Quantidade', color='Quantidade', 
                         color_continuous_scale='Viridis', title="Alunos por Turma")
    ...
    with col2:
        ...
        if alunos.data:
            fig = go.Figure(data=[go.Pie(
                labels=['Em dia', 'Débito'],
                values=[total_alunos - alunos_debito, alunos_debito],
    ...
    if ultimas.data:
        for p in ultimas.data:
            ...
            <div style="padding: 0.5rem; border-bottom: 1px solid #e5e7eb;">
                ✅ <strong>{p.get('nome_aluno', 'Aluno')}</strong> - Entrada registada em {p.get('data', 'N/A')} às {p.get('hora_entrada', 'N/A')}
            </div>
            """, unsafe_allow_html=True)
    else:
            ...
            if st.button("📱 Gerar QR Code", use_container_width=True):
                with st.spinner("Gerando QR code..."):
                    caminho = gerar_qr_code_aluno(
                        ...
                        nome=aluno['nome'],
                        turma=f"{aluno.get('classe', '')} {aluno.get('turma', '')}",
                        escola_id=escola_id
                    )
                    ...
                    if os.path.exists(caminho):
                        ...
                        with open(caminho, "rb") as file:
                            st.download_button(
                                ...
                                data=file,
                                file_name=f"qr_{aluno['username']}.png",
                                mime="image/png"
                            )
                        st.markdown('</div>', unsafe_allow_html=True)
                    else:
    ...
    with tab2:
        ...
        if alunos.data:
            ...
            if st.button("💳 Gerar QR Code de Pagamento", use_container_width=True):
                with st.spinner("Gerando QR code..."):
                    caminho = gerar_qr_code_pagamento(
                        username=aluno['username'],
                        nome=aluno['nome'],
                        valor=valor,
                    ...
                    if os.path.exists(caminho):
                        ...
                        with open(caminho, "rb") as file:
                            st.download_button(
                                label="📥 Baixar QR Code de Pagamento",
                                data=file,
                                file_name=f"qr_pagamento_{aluno['username']}.png",
                                mime="image/png"
                            )
                        st.markdown('</div>', unsafe_allow_html=True)
                    else:
...
def financeiro_page(user, supabase, escola_id):
    ...
    with tab1:
        ...
        with col2:
            st.metric("Alunos com Débito", alunos_debito)
